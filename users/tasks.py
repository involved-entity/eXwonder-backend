...
def send_mail_with_subject_and_body_as_html(
    subject_template: str, body_template: str, recipient_mail: str, context: typing.Optional[typing.Dict] = None
) -> None:
    subject = render_to_string(subject_template).replace("\n", "")
    html_message = render_to_string(body_template, context)
    plain_message = strip_tags(html_message)

    send_mail(subject, plain_message, None, [recipient_mail], fail_silently=True, html_message=html_message)
    return
...
def send_reset_password_mail(
    ...
    PasswordResetForm().send_mail(
        email_template_name, subject_template_name, context, from_email, to_email, html_email_template_name
    )
    return
